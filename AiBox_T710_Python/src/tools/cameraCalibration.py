...
def CalibrationCamera(
    ...
    对图像中的特征点如标定板角点进行检测，得到标定板角点的像素坐标值，
    根据已知的棋盘格大小和世界坐标系原点，计算得到标定板角点的物理坐标值；
    ...
    with open("../../res/calibration/assessment.txt", "a") as f:
        ...
        coefficients = "相机畸变系数: " + str(distortionCoefficients)
        f.write("\n")
        f.write(averageerror)  # 写入平均误差
        f.write("\n")
        f.write(internalmatrix)  # 写入内参矩阵
        f.write("\n")
        f.write(coefficients)  # 写入相机畸变系数
        f.write("\n")
        f.close()
...
def DedistortionDeal(
    ...
    :param distortionCoefficients: 畸变参数
    """
    images = glob.glob(inputFolder + os.sep + "*." + imageType)  # 获取指定文件夹下的指定文件
    ...
    for index, fileName in enumerate(images):
        ...
        imageCorrect = cv2.undistort(
            image, internalMatrix, distortionCoefficients, None, internalMatrix
        )  # 变换图像以补偿镜头畸变
...
if __name__ == "__main__":
    ...
    calibrationPlateSize = (11, 8)  # opencv输入参数为内角点个数
    latticeLength = 0.02  # 格点长度20mm
    imageType = "jpg"  # 图像类型
    (internalMatrix, distortionCoefficients, averageError,) = CalibrationCamera(
        calibrationPlateSize, latticeLength, inputFolder, imageType, outputFolder
    )  # 相机标定
    ...
    print(
        "[["
        + str(round(internalMatrix[0][0], 3))
        + ","
        + str(round(internalMatrix[0][1], 3))
        + ","
        + str(round(internalMatrix[0][2], 3))
        + "] ["
        + str(round(internalMatrix[1][0], 3))
        + ","
        + str(round(internalMatrix[1][1], 3))
        + ","
        + str(round(internalMatrix[1][2], 3))
        + "] ["
        + str(round(internalMatrix[2][0], 3))
        + ","
        + str(round(internalMatrix[2][1], 3))
        + ","
        + str(round(internalMatrix[2][2], 3))
        + "]]",
        end="",
    )
    print(", 畸变参数:", end="")
    print(
        "[["
        + str(round(distortionCoefficients[0][0], 3))
        + ","
        + str(round(distortionCoefficients[0][1], 3))
        + ","
        + str(round(distortionCoefficients[0][2], 3))
        + ","
        + str(round(distortionCoefficients[0][3], 3))
        + ","
        + str(round(distortionCoefficients[0][4], 3))
        + "]]",
        end="",
    )
    ...
    root.append(creatCoeffsTree(distortionCoefficients))
